...
impl CoreVpnApp {
    ...
    /// Build the main UI.
    pub fn build_ui(&self) -> impl Widget + use<> {
        let state = self.state.clone();
        let current_view = state.read().current_view;

        // Build components first to avoid borrow issues
        let header = build_header(state.clone());
        let content = build_main_content(state.clone(), current_view);
        let nav = build_navigation(state);
...
fn build_header(state: SharedState) -> impl Widget {
...
fn build_main_content(state: SharedState, view: AppView) -> Column {
    match view {
        AppView::Connection => build_connection_view(state),
...
fn build_connection_view(state: SharedState) -> Column {
...
fn build_navigation(state: SharedState) -> impl Widget {
    let current_view = state.read().current_view;

    let nav_home = state.clone();
    let nav_servers = state.clone();
    let nav_settings = state;
    ...
    Row::new()
        ...
        .child(nav_button("🏠", "Home", current_view == AppView::Connection, move || {
            nav_home.write().current_view = AppView::Connection;
        }))
        .child(nav_button("📡", "Servers", current_view == AppView::ServerList, move || {
            nav_servers.write().current_view = AppView::ServerList;
        }))
        .child(nav_button("⚙️", "Settings", current_view == AppView::Settings, move || {
...
fn nav_button<F>(icon: &str, label: &str, active: bool, on_click: F) -> impl Widget
where
